---
name: forkit
description: Use when an agent is asked to work with forkit embedded implementation prototypes, run an implementation pass in sibling .forkit.* files, coordinate multiple humans or agents exploring variants, or promote/discard forkit drafts while keeping the canonical source file protected until selection.
---
...
- Treat `*.forkit.*` files as scratch space.
...
- Reuse existing project helpers, components, utilities, hooks, styles, and local patterns before introducing new ones.
